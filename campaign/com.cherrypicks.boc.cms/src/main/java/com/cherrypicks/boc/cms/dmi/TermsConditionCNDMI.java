...
public class TermsConditionCNDMI extends BasicDataSource {
	private static final long serialVersionUID = 8981193072447870139L;
	
	private static final String TERMS_CONDITION_CN = "terms.condition.file.cn";
        ...
        try {
	        ...
	        if (operationType.equals("fetch")) {
	            dsResponse = this.executeFetch(dsRequest);
	        		...
	        		final String binaryResponse = this.getBinaryResponse(TERMS_CONDITION_CN);
	        		bytes = binaryResponse != null ? binaryResponse.getBytes() : null;
	        		result.put("file_cn", bytes);
	        		result.put("file_cn_filesize", (bytes == null) ? 0 : bytes.length);
	        	    dsResponse.setData(result);
	        } else if (operationType.equals("add") || operationType.equals("update")) {
	            
	            item = dsRequest.getUploadedFile("newFile_cn");
    ...
    public DSResponse executeFetch(DSRequest dsRequest) throws Exception {
        ...
        // read  file
        String desc = getBinaryResponse(TERMS_CONDITION_CN);
        if (null != desc) {
        	final String file = PropertiesUtil.getProperty(TERMS_CONDITION_CN);
	        if (StringUtils.isEmpty(file)) {
	        	throw new IllegalArgumentException("Can not find \"terms.condition.file\" in config file.");
	        }
	        mapRecord.put("file_cn", desc.getBytes());
	        mapRecord.put("file_cn_filename", file);
	        mapRecord.put("file_cn_filesize", (desc.getBytes()).length);
	        mapRecord.put("file_cn_date_created", new Date());
	        mapRecord.put("file_cn_description", desc);
        }
    ...
    public DSResponse executeUpdate(DSRequest dsRequest) throws Exception {
        ...
        if (null != fileData) {
	        final String fileName = PropertiesUtil.getProperty(TERMS_CONDITION_CN);
	        ...
	        updateValues.put("file_cn", fileData);
	        updateValues.put("file_cn_filename", fileName);
	        updateValues.put("file_cn_filesize", fileData.length);
	        updateValues.put("file_cn_date_created", new Date());
        }
