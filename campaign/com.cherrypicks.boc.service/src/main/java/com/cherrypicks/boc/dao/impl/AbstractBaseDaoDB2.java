...
import com.cherrypicks.boc.jdbc.JdbcComponent;

public abstract class AbstractBaseDaoDB2<T extends AbstractEntity> extends JdbcComponent {
	...
	public void init() {
		...
		stringBuilder.append(DBSchemaEnum.BOC_CAMPAIGN.toString());
		stringBuilder.append(".");
		stringBuilder.append(this.tableName);
		this.defaultTableName = stringBuilder.toString();
	}
	...
	public T add(T object) {
		...
		object.setId(executeAndReturnKey(param,DBSchemaEnum.BOC_CAMPAIGN.toString()));
	...
	public T addPKObject(T object) {
		SimpleStatementParameter param = new SimpleStatementParameter(this.defaultTableName, "id");
	...
	public boolean del(Object id) throws SQLException {
		String sql = "DELETE FROM " + this.defaultTableName + " WHERE ID = ?";
	...
	public boolean delData(Object id) throws SQLException {
		String sql = "UPDATE " + this.defaultTableName + " SET IS_DELETED=true  WHERE ID = ?";
		StatementParameter param = new StatementParameter();
		//param.setString((String) id);
		param.setObject(id);
		return super.updateForBoolean(sql, param);
	...
	public T update(T object) throws SQLException {
		StringBuilder sql = new StringBuilder("UPDATE " + this.defaultTableName + " SET");
		StatementParameter param = new StatementParameter();
	...
	public T get(Object id) throws SQLException {
		String sql = "SELECT * FROM " + this.defaultTableName + " WHERE ID = ? AND IS_DELETED = ?";
		StatementParameter param = new StatementParameter();
		param.setObject(id);
		param.setObject(0);
		//param.setString((String) id);
	...
	public T defaultGet(Object id) throws SQLException {
		String sql = "SELECT * FROM " + this.defaultTableName + " WHERE ID = ?";
		StatementParameter param = new StatementParameter();
		param.setObject(id);
		//param.setString((String) id);
		return super.query(sql, this.baseModelClass, param);
...
/*	private int getTypes(Object value) {
	...
	public List<T> findByBaseId(Object id) throws SQLException {
		if(existBaseTable) {
			String sql = "SELECT * FROM " + this.defaultTableName + " WHERE " + this.baseTableIdName + " = ? AND IS_DELETED=false";
			StatementParameter param = new StatementParameter();
			//param.setString((String) id);
			param.setObject(id);
	...
	public T getByLang(Object id, String lang) throws SQLException {
		if(existBaseTable) {
			String sql = "SELECT * FROM " + this.defaultTableName + " WHERE " + this.baseTableIdName + " = ? AND LANG = ? AND IS_DELETED=false";
			StatementParameter param = new StatementParameter();
			param.setObject(id);
	...
	public List<T> findByIds(Collection<Object> keys) {
		StringBuilder sql = new StringBuilder("SELECT * FROM ").append(this.defaultTableName).append(" WHERE ID in (:keys)");
	...
	public boolean remove(Object id, String updatedBy) {
		StringBuilder sql = new StringBuilder("UPDATE ").append(this.defaultTableName).append(" SET IS_DELETED = 1, UPDATED_TIME = ?, UPDATED_BY = ? WHERE ID = ?");
		StatementParameter param = new StatementParameter();
	...
	public List<T> findAll() {
		StringBuilder sql = new StringBuilder("SELECT * FROM ").append(this.defaultTableName);
		return super.queryForList(sql.toString(), this.baseModelClass);
	}
	
	public List<T> findAll(int...args) {
		StringBuilder sqlCount = new StringBuilder("SELECT COUNT(*) FROM ").append(this.defaultTableName);
		StringBuilder sql = new StringBuilder("SELECT * FROM ").append(this.defaultTableName);
	...
	public Integer deleteByIds(Collection<Object> keys) {
		StringBuilder sql = new StringBuilder("DELETE FROM ").append(this.defaultTableName).append(" WHERE ID in (:keys)");
		Map<String, Collection<Object>> paramMap = Collections.singletonMap("keys", keys);
